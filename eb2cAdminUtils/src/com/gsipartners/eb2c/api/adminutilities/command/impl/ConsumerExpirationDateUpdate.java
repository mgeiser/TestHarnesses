...
public class ConsumerExpirationDateUpdate implements UtilitiesCommand {

	/**
	 * Logger.
	 */
	private static Logger logger = Logger.getLogger(ConsumerExpirationDateUpdate.class);
	
	@Override
	public void runCommand(Eb2cAPIAdminUtilitiesPropertiesDO pPropertiesDO) throws Eb2cAPIAdminUtilitiesException {
		
        logger.info("-----------------------------------------------------------------------------------------");
        logger.info("Beginning execution of ConsumerExpirationDateUpdate.runCommand()");
        logger.info("-----------------------------------------------------------------------------------------");
		...
		executionContext.ServiceURI.append(UtilityCommandConstants.COMMAND_CONSUMER_UPDATE_URI)
		...
		.append(UtilityCommandConstants.PARAMETER_CATTRIBUTE1).append(UtilityCommandConstants.CONTENT_STRING_EQUALS).append(executionContext.propertiesDO.getKeyExpirationDate());
		
		// Execute the service call
		executionContext = commonServiceCalls.executeServiceCall(executionContext, "Update properties of the specified Consumer id on target Key Manager");

        
        logger.info("-----------------------------------------------------------------------------------------");
        logger.info("Completed execution of ConsumerExpirationDateUpdate.runCommand()");
        logger.info("-----------------------------------------------------------------------------------------");
	...
	public void validateProperites(Eb2cAPIAdminUtilitiesPropertiesDO pPropertiesDO)	throws Eb2cAPIIllegalArgumentsException {
		...
		ParameterValidations.validateIsNotZeroLengthString("keyExpirationDate", pPropertiesDO.getKeyExpirationDate());
