...
public class ConsumerRevoke implements UtilitiesCommand {

	/**
	 * Logger.
	 */
	private static Logger logger = Logger.getLogger(ConsumerRevoke.class);
	
	@Override
	public void runCommand(Eb2cAPIAdminUtilitiesPropertiesDO pPropertiesDO) throws Eb2cAPIAdminUtilitiesException {
		
        logger.info("-----------------------------------------------------------------------------------------");
        logger.info("Beginning execution of ConsumerAttributes.runCommand()");
        logger.info("-----------------------------------------------------------------------------------------");
		...
		executionContext.ServiceURI.append(UtilityCommandConstants.COMMAND_CONSUMER_REVOKE_URI)
		...
		executionContext = commonServiceCalls.executeServiceCall(executionContext, "Revoke access for the specified Consumer Id on target Key Manager");
        
        
        logger.info("-----------------------------------------------------------------------------------------");
        logger.info("Completed execution of ConsumerAttributes.runCommand()");
        logger.info("-----------------------------------------------------------------------------------------");
